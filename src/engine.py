...
def assign_staff_to_shifts_csp(shifts: List[Shift], staff_list: List[Staff]):
    ...
    # 4. Hard Constraint: The Rest Rule (PM -> Any Tomorrow)
    for s_idx, current_shift in enumerate(shifts):
        if current_shift.type in [ShiftType.WEEKDAY_PM, ShiftType.WEEKEND_PM, ShiftType.PUBLIC_HOL_PM]:
            for next_s_idx, next_shift in enumerate(shifts):
                if (next_shift.date - current_shift.date).days == 1:
                    for staff in staff_list:
                        model.Add(assignments[(staff.name, s_idx)] + assignments[(staff.name, next_s_idx)] <= 1)

    # 5 & 6. Hard Constraints: Roles
    for s_idx, shift in enumerate(shifts):
        is_pm = shift.type in [ShiftType.WEEKDAY_PM, ShiftType.WEEKEND_PM, ShiftType.PUBLIC_HOL_PM]
        is_weekday = shift.date.weekday() < 5 
        
        for staff in staff_list:
            if is_pm and staff.role == Role.NO_PM:
                model.Add(assignments[(staff.name, s_idx)] == 0)
            if is_weekday and staff.role == Role.WEEKEND_ONLY:
                model.Add(assignments[(staff.name, s_idx)] == 0)
    
    # --- SOFT CONSTRAINTS & PENALTIES ---
    total_penalties = []
    
    # 8. Point Balancing (YTD Points) - AGGRESSIVE BALANCING
    SCALE = 10 
    total_points_to_assign = sum(s.type.points for s in shifts) * SCALE
    total_existing_ytd = sum(st.ytd_points for st in staff_list) * SCALE
    target_avg = round(int((total_points_to_assign + total_existing_ytd) / len(staff_list)), 1)

    for staff in staff_list:
        # Fixed: Define staff_new_pts correctly
        staff_new_pts = sum(assignments[(staff.name, s_idx)] * int(shifts[s_idx].type.points * SCALE) for s_idx in range(len(shifts)))
        
        starting_pts = int(staff.ytd_points * SCALE)
        staff_total = starting_pts + staff_new_pts

        safe_name = "".join(filter(str.isalnum, staff.name))
        # Range increased to 20000 to handle large YTD totals
        p_dev = model.NewIntVar(0, 20000, f"p_dev_{safe_name}") 
        model.Add(p_dev >= staff_total - target_avg) 
        model.Add(p_dev >= target_avg - staff_total)
        
        # WEIGHT INCREASED: 100 makes fairness the top priority after Bidding
        total_penalties.append(p_dev * 100) 

    # # 9. Weekend Avoidance (Standard Staff)
    # # WEIGHT REDUCED: Allows standard staff to take weekends if fairness requires it
    # for s_idx, shift in enumerate(shifts):
    #     if shift.date.weekday() >= 5:
    #         for staff in staff_list:
    #             if staff.role != Role.WEEKEND_ONLY:
    #                 total_penalties.append(assignments[(staff.name, s_idx)] * 2)
...
# Public Holiday (PH) Mechanism
# Public Holidays are treated as special events outside the normal rotation logic:

# 1.  **Voluntary First:** Slots are opened for bidding first.
# 2.  **Lottery Second:** If unfilled, a random ballot is conducted among eligible staff.
# 3.  **Exemption:** Personnel who complete a PH duty are granted "PH Immunity" for **[X] years**, removing them from future PH lotteries.
# 4.  **Points:** PH duties still award points toward the user's annual total, reducing their likelihood of being assigned normal weekend duties.



    # 10. Bidding System - MAXIMUM PRIORITY (FLAW, staff who gets the bids will be in order of staff_list
    for s_idx, shift in enumerate(shifts): # ONLY FOR PH
        shift_str = shift.date.strftime("%Y-%m-%d")
        if shift.type in [ShiftType.PUBLIC_HOL_AM, ShiftType.PUBLIC_HOL_PM]:
            for staff in staff_list:
                bids = staff.bidding_dates
                if bids:
                    if staff.PH_Immunity: # Ineligble for PH shifts 
                        continue
                    else:
                        bid_strings = [d.strftime("%Y-%m-%d") for d in bids]
                        if shift_str in bid_strings:
                            # Massive reward to ensure 100% success rate
                            total_penalties.append(assignments[(staff.name, s_idx)] * -50000)
